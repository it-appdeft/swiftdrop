			...
			<PageContainer width="full">
				<div className="mb-6 flex items-center gap-3">
					<Button variant="ghost" size="icon-sm" asChild>
						<Link href="/admin/banners"><ArrowLeft /></Link>
					</Button>
					<PageHeader eyebrow="Admin" title="New Banner" />
				</div>
				...
				<form onSubmit={submit}>
								...
								<div className="flex items-center gap-4">
									{preview ? (
										<div className="relative">
											<img src={preview} alt="Preview" className="size-20 rounded-md border border-border/60 object-cover" />
											<button type="button" onClick={() => onPickImage(null)} className="absolute -right-1.5 -top-1.5 flex size-5 items-center justify-center rounded-full bg-destructive text-destructive-foreground shadow-sm" aria-label="Remove image">
												<X className="size-3" />
											</button>
										</div>
									...
									<div className="space-y-2">
										...
										<Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}>{preview ? 'Replace image' : 'Upload image'}</Button>
					...
					<div className="mt-6 flex items-center justify-end gap-3">
						...
						<Button type="submit" loading={processing}>Create banner</Button>
					</div>
				</form>
			</PageContainer>
		</AppLayout>

...
import { Building2, CheckCircle2, Home, MapPin, Minus, Pencil, Plus, Tag, Ticket, Users, UtensilsCrossed, X } from 'lucide-react';
...
function AddressCard({ address, selected, onSelect }: { address: CheckoutAddress; selected: boolean; onSelect: () => void }) {
    ...
    return (
        <button
            ...
            onClick={onSelect}
            className={
                'bg-background flex items-start justify-between gap-3 rounded-2xl border p-4 text-left transition ' +
                (selected ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-zinc-200 hover:border-emerald-300')
            }
        >
...
function CheckoutItemRow({
    ...
    return (
        <div className="flex gap-3 py-3">
            ...
            <div className="flex h-8 items-center gap-2 self-center rounded-md border border-emerald-500 px-1.5">
                <button type="button" aria-label="Decrease" onClick={onDecrement} className="text-emerald-600">
                    <Minus className="size-3.5" />
                </button>
                <span className="min-w-4 text-center text-sm font-bold text-emerald-700">{item.quantity}</span>
                ...
                <div className="p-5">
                    ...
                    <div className="mt-4 space-y-3">
                        {coupons.length === 0 ? (
                            ...
                            coupons.map((c) => <CouponCard key={c.code} coupon={c} onApply={() => onApply(c.code)} />)
                        )}
                    </div>
                </div>
            </DialogContent>
